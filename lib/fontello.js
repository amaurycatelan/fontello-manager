(function() {
  ...
  var address, default, apiRequest, fontello, fs, needle, open, path, print, unzip;
        ...
        if (options.css && options.font) {
          ...
          return zipFile.pipe(unzip.Parse()).on('entry', (function(entry) {
            ...
            if (type === 'File') {
              ...
              if (options.cssname) {
                cssname = fileName.replace(default.style, options.cssname);
              }

              if (options.fontname) {
                fontname = fileName.replace(default.font, options.fontname);
              }

              switch (true) {
                case (dirName === 'css' && fileName === default.style + '.css'):
